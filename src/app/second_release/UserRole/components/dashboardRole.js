  ...
  return (
    <div className="flex gap-2">
      {Object.keys(dropdowns).map((dropdown) => (
        ...
        //   ? " text-BlackHomz "
        //             : ""
            className={ `${selectedOptions[dropdown] ? "text-BlackHomz bg-blue-50":"text-GrayHomz2"}  h-[44px] px-3 adminCellBorders w-[318px] md:w-[346px] p-1 rounded cursor-pointer  ${
              dropdowns[dropdown] ? "adminCellBorders " : ""
            }`}
            onClick={() => handleToggleDropdown(dropdown)}
          >
            <div className={`flex text-[14px] h-full font-[500] ${selectedOptions[dropdown] ? "text-BlackHomz":"text-GrayHomz2"} justify-between items-center `}>
          ...
          {dropdowns[dropdown] && (
            <div className="absolute text-[14px] font-[500] text-BlackHomz mt-2 w-full bg-white rounded-md shadow-md">
              {options[dropdown].map((option) => (
                <div
                  key={option.id}
                  className="p-2 pl-4 cursor-pointer hover:bg-BlueHomz rounded-md hover:text-white text-BlackHomz"
