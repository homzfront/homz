...
const PropertyType = ({ getPropertyType, width }) => {
  ...
  const handleOptionClick = (option, dropdown) => {
    ...
    getPropertyType(option, "PropertyType");
    ...
    <div className="flex gap-2 ">
      {Object.keys(dropdowns).map((dropdown) => (
        <div key={dropdown} className="relative">
          <div
            className={` ${
              width ? `${width} md:w-[139px]` : "w-[176px]"
            } md:min-w-[208px] h-[37px] bg-white text-[#A9A9A9]  adminCellBorders md:mb-1 px-2 pt-2 rounded cursor-pointer  ${
