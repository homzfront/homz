...
const PropertiesPane = ({ data }) => {
  ...
  const handlePageChangeFive = async () => {
    ...
    try {
      ...
      router.push(`/Properties/${profileId}/Tenant`);
    } catch (error) {
  ...
  const fetchProfileId = async () => {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        const currentUrl = window.location.href;
        const profileIdRegex = /\/Profile\/(\d+)/;
        const match = profileIdRegex.exec(currentUrl);
        if (match && match.length > 1) {
          const profileId = match[1];
          resolve(profileId);
  ...
  const handleRowClick = (Profile) => {
    // console.log(Profile);
    router.push(`/Properties/${Profile}/Tenant`);
  };
  return (
    <div className="profiles adminCellBorders flex flex-col rounded-[8px] relative">
      <div className="Taskbar flex flex-wrap md:flex-nowrap gap-[2rem] items-center border-b-2 p-3 md:pl-[2.75rem] h-[70px]">
        ...
        <button
          className={`${activeFive ? "text-blue-600" : "text-BlackHomz"}`}
          onClick={(e) => handlePageChangeFive(e)}
        >
          Tenants
        </button>
      </div>
      <main className="User_body pl-[2.5rem] pt-[2.5rem] pb-[4rem]">
        {loading ? (
          <Loading />
        ) : (
          <>
            ...
            {/* {activeFive && <Link href={`/admin_dashboard/Tenant/${id}`}></Link>} */}
          </>
        )}
      </main>
    </div>
