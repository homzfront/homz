...
const LandByLocation = ({ params }) => {
  ...
  useEffect(() => {
    const routeFilters = parseUrlToFilters(params, 'land');
    
    // Only update if values actually changed
    const filtersChanged = JSON.stringify(filters) !== JSON.stringify(routeFilters);
    
    if (filtersChanged) {
      console.log('[Land/Location Page] Filters changed, updating...');
