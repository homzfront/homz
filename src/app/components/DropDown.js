...
const DropDown = () => {
  ...
  return (
    <div className="flex gap-2">
      <p className="text-[16px] font-[400] text-black pt-1">Filter by:</p>

      {Object.keys(dropdowns).map((dropdown) => (
        <div key={dropdown} className="relative" ref={dropdownRef}>
          <div
            className={`text-black px-4 adminBorders mb-1 p-1 rounded cursor-pointer  ${
              dropdowns[dropdown] ? "adminBorders" : ""
            }`}
            onClick={() => handleToggleDropdown(dropdown)}
          >
            <div className="flex text-[14px] font-[500] text-gray-400 justify-between items-center">
              <span className="mr-2">
          ...
          {dropdowns[dropdown] && (
            <div className="absolute text-[14px] font-[500] text-gray-600 mt-2 w-full bg-white rounded-md shadow-md">
              {options[dropdown].map((option) => (
                <div
                  key={option.id}
                  className="p-2 pl-4 cursor-pointer hover:bg-blue-600 rounded-md hover:text-white"
