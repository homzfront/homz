...
const Activities = () => {
    ...
    useEffect(() => {
        const fetchData = async (page) => {
            ...
            try {
                if (selectedDate) {
                    const response = await api.get(`/wallet/activies/property-owner?page=${page}&startDate=${selectedDate}`);
                    const result = response?.data;
                    setData(result?.data);
                    setTotalPages(result?.pagination?.totalPages);
                    setLoading(false);
                } else {
                    const response = await api.get(`/wallet/activies/property-owner?page=${page}`);
                    const result = response?.data;
                    setData(result?.data);
                    setTotalPages(result?.pagination?.totalPages);
                    setLoading(false);
    ...
    return (
        <div className='p-6'>
            <div className='flex gap-3 items-center'>
                <Link
                    href={"/dashboard/property-owner/payments?tab=wallet"}
