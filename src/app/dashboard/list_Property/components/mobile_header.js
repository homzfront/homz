...
import { mobileSideBarData } from "./sideBarData";
    ...
    >
      ...
      >
        {mobileSideBarData.map((item, idx) => {
          const isLastItem = idx === mobileSideBarData.length - 1;

          return (
            <div key={idx} className="w-full z-50 ">
              {item.submenu ? (
                ...
                <MenuItem>
                  <Link
                    href={item.path}
                    onClick={() => toggleOpen()}
                    className={`flex w-full z-[100px] text-2xl ${
...
const MenuToggle = ({ toggle }) => (
  <button
    onClick={toggle}
    className="pointer-events-auto absolute pt-11 right-[54px] pr-0 top-[6px] py-[.5rem] z-30"
  >
    <svg width="30" height="22" viewBox="0 0 23 23">
      <Path
        variants={{
