...
const Widget = ({ data, setLoadProfile }) => {
  ...
  const ActiveComponent = pages.find((page) => page.id === active)?.component;
    ...
    <div>
      <div className="w-auto h-auto py-4">
            ...
            >
              <p className="text-[14px] font-500">
                ...
                {page.name === "Pay Yearly" && (
                  <span
                    className={`ml-1 py-1 px-2 rounded-md font-normal text-[11px] ${
                      active === page.id ? "bg-white text-BlueHomz" : "bg-BlueHomz text-white"
                    }`}
                  >
                    Save 20%
                  </span>
        ...
        <div className="my-5 rounded-[12px]">
          {ActiveComponent && <ActiveComponent data={data} setLoadProfile={setLoadProfile} />}
        </div>
      </div>
    </div>
  );
