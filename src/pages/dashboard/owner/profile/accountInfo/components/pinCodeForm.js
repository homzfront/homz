...
import { ownerGetOtpPincode, ownerUpdatePincode } from '@/api/propertyService';
...
const PinCodeForm = ({ closeForm }) => {
    ...
    const getOTP = async (e) => {
        ...
        if (password.length < 8) {
            ...
            setLoading(true);
            try {
                const { success, upDateddata, error } = await ownerGetOtpPincode(
                    password,
                );
                if (success) {
                    setLoading(false);
    ...
    const handlePinChnage = async () => {
        if (otp.length !== 4) {
            ...
            setLoading(true);
            try {
                const { success, upDateddata, error } = await ownerUpdatePincode(
                    password,
                    otp,
                    pincode
                );
