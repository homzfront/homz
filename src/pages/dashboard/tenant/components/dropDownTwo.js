    ...
    <div className={`relative inline-block ${className}`}>
      ...
      >
        <div className="flex items-center justify-between">
          <span className={`truncate mr-2 ${selectOption ? "text-GrayHomz2" : "text-BlackHomz"}`}>{selectedOption?.label || selectOption }</span>
      ...
      {isOpen && (
        <div className="overflow-auto h-[100px] scrollbar-container absolute z-20 top-14 w-full text-GrayHomz2 text-[14px] bg-white rounded-md shadow-md max-h-[240px] overflow-y-auto scrollbar-container">
          {options.map((option) => (
            <div
              key={option.id}
