...
import { tenantGetOtpPincode, tenantUpdatePincode } from '@/api/tenantSevice';
...
const PinCodeForm = ({ closeForm }) => {
    ...
    const getOTP = async (e) => {
        ...
        if (password.length < 8) {
            ...
            setLoading(true);
            try {
                const { success, upDateddata, error } = await tenantGetOtpPincode(
                    password,
                );
                if (success) {
                    setLoading(false);
    ...
    const handlePinChnage = async () => {
        if (otp.length !== 4) {
            ...
            setLoading(true);
            try {
                const { success, upDateddata, error } = await tenantUpdatePincode(
                    password,
                    otp,
                    pincode
                );
