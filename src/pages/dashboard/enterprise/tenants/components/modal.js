...
const Modal = ({ setInviteTenant, dropdownRef }) => {
  ...
  const handleGetLink = async (e) => {
    e.preventDefault();
    if (loadingII) return; // Do nothing if already loading

    setLoadingII(true); // Set loading to true when submitting the form
            ...
            <div className="mt-[-15px] rounded-md">
              <div className="flex border justify-between h-[48px] bg-whiteblue rounded-md p-2 items-center mb-2">
                ...
                <span onClick={handleCopyClick}>
                  ...
                  <Image
                    src={
                      "/static/dashboard/enterprisemanager/dashboard/copy.png"
                    }
                    height={16}
                    width={16}
                    alt=""
