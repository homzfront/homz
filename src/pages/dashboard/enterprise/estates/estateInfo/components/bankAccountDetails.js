...
import { updateEstateBankInfo } from '@/api/estateService';
...
const BankAccountDetails = ({ data }) => {
    ...
    useEffect(() => {
        if (data) {
            ...
            setAccountNo((data?.bankInfo?.accountNumber) || "");
    ...
    useEffect(() => {
        ...
        fetchBankName();
    }, [accountNo, bankName, defaults]);

    const updateDone = async (e) => {
        e.preventDefault();
        if (accountName === "") {
            setErrorName("Fill in bank details")
            return;
        }
        if (loading) return;
        setLoading(true);
        ...
        try {
            ...
            const { success, error } = await updateEstateBankInfo(
                data._id,
                updatedBankInfo
            );
            if (success) {
        ...
        <div className="w-full">
            {
                loading ? (
                    ...
                    <div className={`border-t w-full p-8 ${loading ? "pointer-events-none" : ""}`}>
                            ...
                            <div className='grid grid-cols-1 md:grid-cols-2 w-full'>
                                ...
                                <div className="w-[100%]">
                                    ...
                                    <div
                                        ...
                                        {
                                            loadingBank ? <LoadingForm /> : errorName === "Could not resolve account name. Check parameters or try again." ? "Account Name" : accountName ? accountName : "Account Name"}
                                    </div>
                                </div>
                            </div>
                            {
