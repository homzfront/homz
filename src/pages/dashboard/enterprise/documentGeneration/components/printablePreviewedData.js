...
const PrintablePreviewedData = ({ printableRef, formData }) => {
    return (
        <div ref={printableRef} className=''>
            <div className='h-screen px-[48px] py-[70px] bg-white flex justify-center items-center'>
                <div className='flex flex-col gap-4 items-center justify-center w-full md:w-auto px-4 md:px-0'>
                    ...
                    <div className='h-[50px] bg-inputBg w-full md:w-[480px] text-[20px] font-[500] text-BlackHomz flex justify-center items-center rounded-[4px]'>
                    ...
                    <div className='w-full md:w-auto'>
                        ...
                        <div className='h-[65px] rounded-[4px] border w-full md:w-[480px] text-[16px] font-[400] text-GrayHomz p-2'>
                            {formData?.propDesc ? formData?.propDesc : ""}
                        </div>
                    </div>
                </div>
            </div>
            <div className='page-break h-screen px-[48px] py-[70px] bg-white flex justify-center items-center'>
                <div className='h-[90%] bg-white flex flex-col text-[16px] font-[400] text-BlackHomz text-justify justify-between'>
                    ...
                    <p><span className='text-BlackHomz font-[600]'>{formData?.landlordName ? formData?.landlordName : ""}</span>, of <span className='text-BlackHomz font-[600]'>{formData?.landlordAddress ? formData?.landlordAddress : ""}</span>, (Hereinafter referred to as "THE LANDLORD" which expression shall where the context so admits include his heir(s), executors, administrators and assigns) of the one part.</p>

                    <p>AND</p>

                    <p><span className='text-BlackHomz font-[600]'>{formData?.tenantName ? formData?.tenantName : ""}</span>, of <span className='text-BlackHomz font-[600]'>{formData?.tenantAddress ? formData?.tenantAddress : ""} </span>, (Hereinafter referred to as "THE TENANT" which expression shall where the context so admits include his heirs and successor in title) of the other part.</p>
                    ...
                    <p>WHEREAS</p>
                    <div>
                        <p>1. The Landlord is a beneficial owner of the property situate at  <span className='font-[600] text-BlackHomz'>{formData?.propAddress ? formData?.propAddress : ""} </span>, herein regarded as  <span className='font-[600]'>“The Demised Premises”</span>.</p>
                        <p>2. The Landlord has agreed to rent out all the  <span className='font-[600] text-BlackHomz'> {formData?.propDesc ? formData?.propDesc : ""}</span> with all appurtenances to the Tenant and the tenant has agreed to take same.</p>
            ...
            <div className='page-break h-screen px-[48px] py-[70px] bg-white flex justify-center items-center'>
                <div className='h-[90%] bg-white flex flex-col text-[16px] font-[400] text-BlackHomz text-justify justify-between'>
                    ...
                    <p>2.1	In consideration for the grant by the <span className='font-[600]'>Landlord</span>, of a tenancy in respect of the <span className='font-[600]'>Demised Premises</span> for the term reserved herein, the <span className='font-[600]'>tenant</span> shall pay to the <span className='font-[600]'>Landlord</span>, upon execution of this agreement the sum of <span className='font-[600] text-BlackHomz'>{formData?.rentPaymentInWords ? formData?.rentPaymentInWords : ""}, ({formData?.selectedCurrency ? extractCurrencySymbol(formData?.selectedCurrency) : ""} {formData?.rentPayment ? addCommasToNumberWithoutN(formData?.rentPayment) : ""})</span> per annum (g) net of all taxes, levies, all fully paid in advance, the receipt whereof the Landlord hereby acknowledges.</p>
            ...
            <div className='page-break h-screen px-[48px] py-[70px] bg-white flex flex-col text-[16px] font-[400] text-BlackHomz text-justify gap-6'>
...
export default PrintablePreviewedData