...
const Dropdown = ({ options, onSelect, selectOption, className }) => {
  ...
  return (
    <div className={`relative inline-block  w-full ${className}`}>
      <div
        className={`text-BlackHomz  px-4 border h-[42px] p-2 rounded-md cursor-pointer ${
          isOpen ? "border" : ""
        }`}
        onClick={handleDropdownToggle}
      >
        <div className={`flex items-center justify-between`}>
          <span className={`mr-2 ${selectOption ? "text-GrayHomz2" : "text-BlackHomz"}`}>
      ...
      {isOpen && (
        <div className="w-full absolute z-20 top-14 text-GrayHomz2 text-[14px] bg-white rounded-md shadow-md">
