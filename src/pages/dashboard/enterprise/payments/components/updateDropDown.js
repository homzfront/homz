...
const UpdateDropDown = ({ options, onSelect, selectOption, className }) => {
  ...
  return (
    <div className={`relative inline-block ${className}`} ref={dropdownRef}>
      <div
        className={`text-BlackHomz px-4 border min-h-[45px] p-3 rounded-md bg-white cursor-pointer ${
          isOpen ? "border" : ""
        }`}
        onClick={handleDropdownToggle}
      >
        <div className="flex items-center justify-between">
          <span className={`mr-2 ${!selectOption ? "text-GrayHomz2" : "text-BlackHomz"}`}>{selectOption || "Select one" }</span>
